...
class UpdateRiderReferences extends Command
{
    public function handle(RiderProfileService $riderProfileService): int
    {
        ...
        RiderProfile::query()
            ...
            ->chunkById(200, function ($riderProfiles) use ($riderProfileService, &$updated, &$missingZone): void {
                foreach ($riderProfiles as $riderProfile) {
                    ...
                    $riderProfile->update([
                        'rider_ref' => $riderProfileService->generateRiderRef($riderProfile->id, $riderProfile->homeZone->code),
                    ]);
        ...
        $this->info("Updated {$updated} rider reference(s); {$missingZone} skipped for missing a home zone code.");
